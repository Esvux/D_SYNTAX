...
public class Gramatica {
    ...
    public String agregarMarcador(ArrayList<Nodo> acciones) {
        ...
        Produccion marcador = Produccion.nuevaProduccion(izq, acciones);
    ...
    private String aumentarGramatica(String raiz) {
        String expandida = raiz + "'";
        Elemento aumentado = Elemento.nuevoElemento(expandida);
        aumentado.marcarComoAumentado();
        aumentado.marcarComoNoTerminal();
        this.agregarElemento(aumentado);
        this.agregarProduccion(Produccion.nuevaProduccion(expandida, this.getElemento(raiz)));
        return expandida;
    }
    ...
    public void cambiarModoLR() {
        ...
        //System.out.println("Primeros calculados...");
        //mostrarPrimeros();
    }

    private void mostrarPrimeros() {
        Iterator<Elemento> it = elementosLR.values().iterator();
        while (it.hasNext()) {
            Elemento next = it.next();
            System.out.print(next.getPresentacion() + ":");
            Iterator<Integer> itPrimeros = next.getPrimeros().iterator();
            while (itPrimeros.hasNext()) {
                Integer primero = itPrimeros.next();
                System.out.print(elementosLR.get(primero).getPresentacion() + (itPrimeros.hasNext() ? ", " : ""));
            }
            System.out.println("");
        }
    }
